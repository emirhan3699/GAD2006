...
void ACOAAvatar::Tick(float DeltaTime)
{
	...
	{
		if (Stamina < MaxStamina && SRR > 0.0f)
		{
			Stamina += SRR * DeltaTime;
	...
	if (GEngine)
	{
		GEngine->AddOnScreenDebugMessage(-1, DeltaTime, FColor::Green, FString::Printf(TEXT("Stamina: %f"), Stamina));
	}
